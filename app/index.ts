...
import { type Url, type User } from "@generated/prisma/client.js";
import { prisma } from "@lib/prisma.js";
import { encodeBase62, decodeBase62 } from "@zipr/app/utils.js";
...
fastify.post('/shorten', async (request: FastifyRequest<{ Body: ShortenBodyType}>, reply) => {
    if (request.body.shortening_type == ShorteningTypes.Auto && request.body.shortUrl !== null)
        reply.code(400).send(fastify.httpErrors.badRequest('Cannot specify shortUrl in auto shorten method'));
    ...
    reply.code(201).send({shortUrl: shortUrl});
});
